...
func (s *Server) GetMembersHandler(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		var metadata []byte
		if err := rows.Scan(&m.ID, &m.UserID, &m.OrgID, &m.Name, &m.Email, &m.Status, &m.Role, &m.Balance, &m.FeeFormula, &metadata, &m.UpdatedAt); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
...
func (s *Server) RegisterMemberHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Vi starter transaksjonen HELT øverst for å sikre konsistens mellom User og Membership
	...
	// 1. Sjekk/Opprett bruker
	var userID string
	err = tx.QueryRowContext(r.Context(), "SELECT id FROM users WHERE email = $1", req.Email).Scan(&userID)
	
	if err == sql.ErrNoRows {
		// Ta lås på email for å unngå race condition ved ny bruker
		_, _ = tx.ExecContext(r.Context(), "SELECT pg_advisory_xact_lock(hashtext($1))", req.Email)
		...
		if err == sql.ErrNoRows {
			...
			if err := s.eventStore.AppendWithTx(r.Context(), tx, userID, 1, userEvent); err != nil {
	...
	// 2. Ta lås på UserID for medlemskap-sjekk
	...
	// 3. Finn Policy og sjekk Hierarkisk Idempotency
	var orgPath string
	...
	if orgPath != "" {
		err = tx.QueryRowContext(r.Context(), `
			SELECT EXISTS (
				SELECT 1 FROM organization_hierarchy 
				WHERE path @> $1 AND (policy->>'allow_multiple')::boolean = false
			)`, orgPath).Scan(&isExclusive)
	}

	if isExclusive {
		var controllingOrgID, controllingOrgName string
		err = tx.QueryRowContext(r.Context(), `
			SELECT id, name FROM organization_hierarchy 
			WHERE path @> $1 AND (policy->>'allow_multiple')::boolean = false
			ORDER BY nlevel(path) ASC LIMIT 1`, orgPath).Scan(&controllingOrgID, &controllingOrgName)
		
		if err != nil {
			http.Error(w, "Policy org not found", http.StatusInternalServerError)
			return
		}

		var conflictID, conflictOrgName string
		// Finn alle medlemskap for denne brukeren, og sjekk om orgen de tilhører er i grenen til controllingOrg
		err = tx.QueryRowContext(r.Context(), `
			SELECT m.id, o.name FROM membership_view m
			JOIN organization_hierarchy o ON m.org_id = o.id
			WHERE m.user_id = $1 AND (
				o.path <@ (SELECT path FROM organization_hierarchy WHERE id = $2)
				OR o.path @> (SELECT path FROM organization_hierarchy WHERE id = $2)
			) LIMIT 1`, 
			userID, controllingOrgID).Scan(&conflictID, &conflictOrgName)
